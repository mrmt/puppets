 ...
 * EmDash のメディアとして featured_image に保存する。表示時に外部へ取りに行くことはしない
 */
import { mediaUrl } from "./media";
import { youtubeId, youtubeThumbnail } from "./youtube";

interface PostLike {
	data: { video_url?: string | null; featured_image?: unknown; photos?: unknown };
}

/** 投稿の OGP 画像: featured_image → 写真 → (未設定の新しい投稿のみ) YouTube サムネイル。無ければ null */
export function ownPostImage(post: PostLike, origin: string): string | null {
	...
	const stored = mediaUrl(post.data.featured_image) ?? mediaUrl(photos[0]);
